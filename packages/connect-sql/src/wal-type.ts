...
import { WALState } from "@fireproof/encrypted-blockstore";
import { DataRecord, DataSQLStore, SQLiteDataStore } from "./data-type";
...
class SQLiteWalStore implements WalSQLStore {
    ...
    async start(): Promise<SQLiteWalStore> {
        ...
        this.insertStmt = this.dbConn.client!.prepare(`insert into ${this.table}
          ...
          `)
        this.selectStmt = this.dbConn.client!.prepare(`select name, branch, state, updated_at from ${this.table} where name = ? and branch = ?`)
    ...
    async insert(ose: WalRecord): Promise<RunResult> {
        ...
        return this.insertStmt!.run(ose.name, ose.branch, bufState, wal.updated_at.toISOString(), bufState, wal.updated_at.toISOString())
