...
import { InCreateTenantParams, FPApiParameters, OutTenantParams, User } from "@fireproof/core-types-protocols-dashboard";
...
export async function insertTenant(
  ...
  const cnt = await ctx.db.$count(sqlTenants, eq(sqlTenants.ownerUserId, auth.userId));
  if (cnt + 1 >= auth.maxTenants) {
