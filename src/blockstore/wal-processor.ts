...
abstract class Action<T> {
    ...
    constructor(loader: Loadable, op: T, logger: Logger) {
        this.loader = loader;
        this.op = op;
        this.logger = logger;
        this.id = uuidv7();
...
class DbMetaAction extends Action<DbMetaOp> {
    ...
    constructor(opStyle: ActionStyle, loader: Loadable, wal: WALStore, op: DbMetaOp, logger: Logger) {
        super(loader, op, ensureLogger(logger, "DbMetaAction"));
...
let seq = 0;
function dbMetaActions(loader: Loadable, wal: WALStore, style: ActionStyle, op: DbMeta, logger: Logger): Action<DbMetaOp>[] {
    const id = uuidv7();
    const mySeq = seq++;
    return op.cars.map(cid => new DbMetaAction(style, loader, wal, {
        seq: mySeq,
        dbMetaId: id,
        dbMeta: op,
        cid
...
class FileAction extends Action<FileOp> {
    ...
    constructor(loader: Loadable, wal: WALStore, op: FileOp, logger: Logger) {
        super(loader, op, ensureLogger(logger, "FileAction"));
        this.walStore = wal;
    }
...
function fileActions(loader: Loadable, wal: WALStore, ops: FileOp[], logger: Logger): Action<FileOp>[] {
    return ops.map(op => new FileAction(loader, wal, op, logger));
}
...
export class WALProcessorImpl implements WALProcessor {
    ...
    constructor(logger: Logger) {
        this.logger = ensureLogger(logger, "WALProcessorImpl");
    }
    ...
    addState(walState: Partial<WALProcessorState>) {
        ...
        for (const op of walState.noLoaderOps || []) {
            this.addActions(keys, dbMetaActions(op.loader, op.walStore, ActionStyle.noLoaderOps, op.op, op.loader.logger));
            // this.walState.noLoaderOps = this.walState.noLoaderOps.filter((op) => op !== dbMeta);
        }
        for (const op of walState.operations || []) {
            this.addActions(keys, dbMetaActions(op.loader, op.walStore, ActionStyle.ops, op.op, op.loader.logger));
            //         this.walState.operations = this.walState.operations.filter((op) => op !== dbMeta);
        }
        for (const op of walState.fileOperations || []) {
            this.addActions(keys, fileActions(op.loader, op.walStore, [op.op], op.loader.logger));
            // this.walState.fileOperations = this.walState.fileOperations.filter((op) => op.cid !== fileCid);
        }
        this.trigger();
    }
    ...
    snapState(loader: Loadable): WALState {
        ...
        const byStyle = actions.reduce((acc, op) => {
    ...
    async trigger() {
        ...
        logger.Debug().Len(pendingActions)/*.Any("styles", pendingActions.map(a => a.opStyle))*/.Msg("triggering actions");
        const resultSettled = await Promise.allSettled(pendingActions.map(a =>
        ...
        const byId = results.reduce((acc, a) => {
...
export function walProcessor(logger: Logger): WALProcessor {
    return once.once(() => new WALProcessorImpl(logger));
}
