import { BuildURI, CoerceURI, Future, isPromise, Logger, Result, runtimeFn, URI } from "@adviser/cement";
import {
  buildReqGestalt,
  defaultGestalt,
  EnDeCoder,
...
class RequestAction<Q extends MsgBase, S extends MsgBase> implements Thenable<MsgWithError<S>> {
  ...
  constructor(realConn: MsgRawConnection, req: Q, opts: RequestOpts) {
    this.req = req;
    this.opts = opts;
    this.realConn = realConn;
  }
  then(): Promise<MsgWithError<S>> {
    return this.realConn.request(this.req, this.opts);
  }
}
...
export class VirtualConnected {
  ...
  readonly activeBinds = new Map<string, ActiveStream<MsgWithConn, MsgBase>>();

  private async handleBindRealConn(
    realConn: MsgRawConnection,
    req: MsgWithOptionalConn,
    opts: RequestOpts,
    as: ActiveStream<MsgBase, MsgBase>,
  ): Promise<void> {
    const stream = realConn.bind<MsgBase, MsgWithConn>({ ...req, conn: (req.conn || this.conn) as QSId }, opts);
    console.log("bind-pre-stream");
    for await (const msg of stream) {
      console.log("bind-got-stream", msg);
      // if (!MsgIsTid(msg, req.tid)) {
      //   break
      // }
      if (MsgIsConnected(msg, this.conn)) {
        if (opts.waitFor?.(msg) || MsgIsError(msg)) {
  ...
  bind<S extends MsgWithConn, Q extends MsgWithOptionalConn>(req: Q, opts: RequestOpts): ReadableStream<MsgWithError<S>> {
    ...
    return new ReadableStream<MsgWithError<S>>({
      start: (ctl) => {
        this.getRealConn(req, async (realConn: MsgRawConnection) => {
          const as = {
            id,
            bind: {
              msg: req,
          ...
          this.handleBindRealConn(realConn, req, opts, as);
          return Result.Ok(undefined);
        }).catch((err: Error) => {
          ctl.error({
            type: "error",
  ...
  request<S extends MsgWithConn, Q extends MsgWithOptionalConn>(req: Q, opts: RequestOpts): Promise<MsgWithError<S>> {
    return this.getRealConn(req, (realConn: MsgRawConnection) =>
      realConn.request<S, Q>(
        {
          ...req,
          conn: req.conn || this.conn,
        },
        opts,
      ),
    );
  ...
  send<S extends MsgWithConn, Q extends MsgWithOptionalConn>(msg: Q): Promise<MsgWithError<S>> {
    return this.getRealConn(msg, (realConn: MsgRawConnection) =>
      realConn.send<S, Q>({
        ...msg,
        conn: msg.conn || this.conn,
      }),
    );
  }

  close(t: MsgWithOptionalConn): Promise<Result<void>> {
    return this.getRealConn(t, (realConn: MsgRawConnection) => realConn.close(t));
  }
  ...
  private async getQSIdWithSideEffect(msg: MsgBase): Promise<MsgWithError<ResOpen>> {
    const mOpen = await this.request<ResOpen, ReqOpen>(buildReqOpen(this.sthis, msg.auth, this.opts.conn), {
  ...
  private async getRealConn<S extends MsgBase, Q extends MsgBase, X extends MsgWithError<S> | Result<void>>(
    ...
    if (!this.realConn) {
      ...
      if (rConn.isErr()) {
        ...
        return this.getRealConn(msg, action);
      }
      this.realConn = rConn.Ok();
      const mQSid = await this.getQSIdWithSideEffect(msg);
      if (MsgIsError(mQSid)) {
        return mQSid as unknown as X;
      }
      for (const as of this.activeBinds.values()) {
        await this.handleBindRealConn(this.realConn, msg, { waitFor: MsgIsResOpen }, as) 
      }
      return this.getRealConn(msg, action);
    } else {
      if (!this.realConn.isReady) {
        await this.realConn.close(msg).then();
