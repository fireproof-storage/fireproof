...
export class V0_19NSWWalStore implements WalSQLStore {
  ...
  constructor(dbConn: DBConnection) {
    ...
    this.logger = ensureLogger(dbConn.opts, "V0_19NSWWalStore");
  ...
  async select(url: URL, key: WalKey): Promise<WalRecord[]> {
    const res = (await this.selectStmt(url).then((i) => i.all({
      ...
      return {
        ...
        state: Uint8Array.from(row.state?.valueOf() as Buffer),
