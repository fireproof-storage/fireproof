...
import { BuildURI, Keyed, KeyedResolvOnce, KeyedResolvSeq, Lazy, Logger, ResolveOnce, Result, sleep } from "@adviser/cement";
import {
  DashApi,
  AuthType,
...
const backendStatePerDb = new Keyed<BackendState, BackendKey>(({ key }: { key: BackendKey }) => {
  ...
  return new ResolveOnce<BackendState>({
    ...key,
    getState: () => state,
...
export class SvcFPCCProtocol implements FPCCProtocol {
  ...
  constructor(sthis: SuperThis, opts: SvcFPCCProtocolOpts) {
    ...
    this.backendFPCC =
      opts.backend ??
      backendPerDashUri.get(this.dashboardURI).once(() => {
  ...
  listRegisteredDbs(): BackendKey[] {
    return Array.from(backendStatePerDb.values())
      .filter((bstate) => bstate.value.Ok().getState() === "ready")
      .map((state) => {
          ...
          if (rAuthToken.isErr()) {
            ...
            if (this.backendFPCC.isFPCCEvtAppReady()) {
              ...
              if (rFpccEvtApp.isErr()) {
                this.logger.Warn().Err(rFpccEvtApp).Any({ appId: state.appId, dbName: state.dbName }).Msg("Backend reports error");
              }
              if (rFpccEvtApp.isOk()) {
                this.logger
                  .Debug()
