...
export class SessionTokenService {
    static async generateKeyPair(alg = "ES256", options = { extractable: true }, generateKeyPairFN = (alg, options) => generateKeyPair(alg, options)) {
    ...
    static async createFromEnv(sthis, sp = {}) {
        ...
        if (!envToken) {
            throw new Error(`env not found for: ${sp.privateEnvKey ?? envKeyDefaults.SECRET} or ${sp.publicEnvKey ?? envKeyDefaults.PUBLIC}`);
        }
