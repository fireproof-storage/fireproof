...
import { V2KeysItem, PARAM, StoreType, KeyBagIf, KeyWithFingerPrint, V2StorageKeyItem } from "@fireproof/core-types-base";
...
import { CryptoAction, IvKeyIdData, Loadable, V2SerializedMetaKey } from "@fireproof/core-types-blockstore";
...
describe("KeyBag", () => {
  ...
  it("default and multiple fingerprints", async () => {
    ...
    if (!("_prepare" in provider)) {
      diskBag = (await provider.get(name)) as V2StorageKeyItem;
    } else {
      if (runtimeFn().isBrowser) {
        const p = provider as KeyBagProviderIndexedDB;
        diskBag = await p._prepare().then((db) => db.get("bag", name));
