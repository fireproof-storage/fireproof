    ...
    for (let i = 0; i < jobs; i++) {
        ref.addState({
            ...
            noLoaderOps: withLoader(loader, walStore, [
                { cars: Array(4).fill(0).map((_, j) => `${i}:noLoaderOp1.${j}` as unknown as AnyLink) },
                { cars: [`${i}:noLoaderOp2` as unknown as AnyLink] },
            ] as DbMeta[]),
            operations: withLoader(loader, walStore, [
                { cars: Array(4).fill(0).map((_, j) => `${i}:op1.${j}` as unknown as AnyLink) },
...
describe("blockstore/wal-processor", () => {
    ...
    it("check once", async () => {
        const ref = walProcessor(logger);
        for (let i = 0; i < 10; i++) {
            expect(ref).toEqual(walProcessor(logger));
        }
    })
    ...
    it("loader without remoteCarStore", async () => {
        const ref = new WALProcessorImpl(logger);
    ...
    it("loader with remoteCarStore", async () => {
        ...
        await createState(ref, loader, walStore)
        expect(ref.snapState(loader)).toEqual({
            fileOperations: Array(10).fill(0).flatMap((_, i) => ([
                { cid: `${i}:fileOp1` as unknown as AnyLink, public: true },
                { cid: `${i}:fileOp2` as unknown as AnyLink, public: false }
            ])),
            noLoaderOps: Array(10).fill(0).flatMap((_, i) => ([
                { cars: Array(4).fill(0).map((_, j) => `${i}:noLoaderOp1.${j}` as unknown as AnyLink) },
                { cars: [`${i}:noLoaderOp2` as unknown as AnyLink] },
            ])),
            operations: Array(10).fill(0).flatMap((_, i) => ([
                { cars: Array(4).fill(0).map((_, j) => `${i}:op1.${j}` as unknown as AnyLink) },
                { cars: [`${i}:op2` as unknown as AnyLink] },
            ])),
        });
    ...
    it("loader process remoteCarStore", async () => {
        ...
        expect(mockFns.fileStore_load.mock.calls).toEqual(Array(10).fill(0).flatMap((_, i) => ([
            [`${i}:fileOp1`], [`${i}:fileOp2`],
        ])))
        ...
        expect(mockFns.carStore_load.mock.calls.filter(i => i[0].includes("noLoaderOp"))).toEqual(Array(10).fill(0).flatMap((_, i) => ([
            [`${i}:noLoaderOp1.0`],
            [`${i}:noLoaderOp1.1`],
            [`${i}:noLoaderOp1.2`],
            [`${i}:noLoaderOp1.3`],
        ...
        expect(mockFns.remoteCarStore_save.mock.calls.filter(i => i[0].cid.includes("noLoaderOp"))).toEqual(Array(10).fill(0).flatMap((_, i) => ([
