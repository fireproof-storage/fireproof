...
class TestConnection implements ps.cloud.MsgRawConnection {
  ...
  readonly activeBinds: Map<string, ps.cloud.ActiveStream<ps.cloud.MsgBase, ps.cloud.MsgBase>>;
  ...
  close(o: ps.cloud.MsgBase): Promise<Result<void>> {
    this.closeFn(o);
    console.log("close", o, this.closeFn.mock.calls);
    return Promise.resolve(Result.Ok(undefined));
  }
...
class MockHttpConnection extends TestConnection implements ps.cloud.MsgRawConnection {
  ...
  activeBinds: Map<string, ps.cloud.ActiveStream<ps.cloud.MsgBase, ps.cloud.MsgBase>>;
...
class MockWSConnection extends TestConnection implements ps.cloud.MsgRawConnection {
  ...
  close(o: ps.cloud.MsgBase): Promise<Result<void>> {
    ...
    console.log("ws-close", o);
    return Promise.resolve(Result.Ok(undefined));
  }
  onMsg(msg: ps.cloud.OnMsgFn<ps.cloud.MsgBase>): ps.cloud.UnReg {
    super.onMsg(msg);
    console.log("ws-onMsg", msg);
...
describe("retry-connection", () => {
  ...
  afterEach(async () => {
    console.log("pre-afterEach", connected);
  ...
  it("bind", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      console.log("bind-send-read");
      const { done, value: msgl } = await reader.read();
      console.log("reader", done, msgl);
      expect(done).toBe(false);
