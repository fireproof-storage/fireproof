...
describe("CRDT with an index", function () {
  ...
  beforeEach(async function () {
    ...
    idx = await index<number, CRDTTestType>(sthis, { crdt: crdt }, "points");
  });
  ...
  it("should register the index", async function () {
    const rIdx = await index<number, CRDTTestType>(sthis, { crdt: crdt }, "points");
  ...
  it("creating a different index with same name should not work", async function () {
    const e = await index(sthis, { crdt: crdt }, "points", (doc) => doc._id)
      .query()
      .catch((err) => err);
