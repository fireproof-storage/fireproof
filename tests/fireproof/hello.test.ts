import { fireproof, DocResponse, DocWithId, index, Database, fireproofDB, isDatabase } from "@fireproof/core";
...
describe("hello public API", () => {
  ...
  beforeEach(async () => {
    await sthis.start();
    db = fireproofDB("test-public-api");
...
describe("Simplified Reopening a ledger", function () {
  ...
  beforeEach(async function () {
    db = fireproofDB("test-reopen-simple");
  ...
  it("should have the same data on reopen", async function () {
    const db2 = fireproofDB("test-reopen-simple");
